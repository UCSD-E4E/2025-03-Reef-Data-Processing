WITH priority_images AS (
  SELECT replace(data_paths.path || images.path, '/mnt/fishsense_data_reef/REEF/data', '/home/ubuntu/data') as paths, images.camera_sn
  FROM images
...
params AS (
  SELECT replace(cameras.lens_cal_path, '//e4e-nas.ucsd.edu/fishsense/Fishsense Lite Calibration Parameters', '/home/ubuntu/lens_cal') as lens_cal_path, cameras.name, paths
  FROM grouped_images
  ...
  'jobs': json_arrayagg(
    json_object(
      ...
      'parameters': json_object(
		...
		'psql-connection-string': 'postgress://ccrutchf@fabricant.ucsd.edu/postgres',
        'format': 'JPG',
        'output': 'label_studio_headtail'
      )
    )
  )
)
